...
- 表名 ≤ 64 字符
- 测点名 ≤ 128 字符
**理由**: 存储路径 `data/hot/<db>/tables/<table>/tags/<tag>/<date>.sst`，Windows MAX_PATH 为 260 字符。
路径固定开销 38 字符，64+64+128 = 256 字符，合计 294 字符略超。因此超过 MAX_PATH 时需使用 `\\?\` 前缀或限制更严（推荐 `\\?\`）。

### 验证规则
- 名称只允许字母、数字、下划线、连字符
- 不允许以 `-` 或 `_` 开头
- 大小写敏感（`BOILER-001` ≠ `boiler-001`）
