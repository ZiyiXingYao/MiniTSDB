...
// FileTime 转 epoch 毫秒
inline int64_t FileTimeToMs(uint32_t low, uint32_t high) {
...
inline int64_t LastWriteTimeMs(const std::string& path) {
#ifdef _WIN32
    WIN32_FILE_ATTRIBUTE_DATA info;
    if (!GetFileAttributesExA(path.c_str(), GetFileExInfoStandard, &info))
        return -1;
...
inline bool ListDirectory(const std::string& path, std::vector<DirEntry>& entries) {
    entries.clear();
